...
export default function ClientSignup() {
    ...
    function handleSubmit(event) {
        ...
        if (!phone) {
            temp.phone = "Phone field is required"
            validation = false
        }
        if (!password) {
                ...
                .catch(err => {
                    ...
                    if (err.response.data.errors && err.response.data.errors.length) {
                        err.response.data.errors.forEach(el => {
                            ...
                            temp_arr.push(el.message)
                        })
                        // setError(temp)
                        setResError(temp_arr)
                    }
                ...
                <div className="container pt-24 pb-24 flex flex-col items-center gap-20">
                    ...
                    <div className="bg-white w-full md:w-2/5 rounded-md">
                        <form className="px-8 py-16" onSubmit={handleSubmit}>
                            ...
                            <button type="submit" disabled={submitting} className="flex disabled:bg-black/70 w-full h-fit justify-center items-center border border-black bg-primary-dark py-2 px-7 text-white hover:text-primary-dark hover:bg-white mt-8 rounded-md">Register</button>
